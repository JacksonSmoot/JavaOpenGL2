...
public class MusicPlayer {
    ...
    public long getElapsedNs() {
        ...
        return basePositionNs + openAL.getPlayedTimeNs(outRate);
    }
    ...
    public void pause() {
        ...
        long playedNs = openAL.getPlayedTimeNs(outRate);
    ...
    public void seekToMs(long ms){
        seekToNs(TimeUnit.MILLISECONDS.toNanos(ms));
    }
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    // ✅ Long-lived copy (direct) - you own this memory
    ...
    public void rewindToStart() {
        ...
        if (ret < 0) {
            throw new RuntimeException("Seek failed: " + ret);
        }
    ...
    private void audioLoop() {
        ...
        try {
            while (running) {
                ...
                openAL.pumpProcessed();
                if (openAL.queuedBufferCount() >= 6) {
                    Thread.sleep(1);
                    continue;
                }
                ...
                if(pkt == AUDIO_EOF_PACKET) {
                    while(openAL.queuedBufferCount() != 0){
                        openAL.pumpProcessed();
                    }
                    while(openAL.isPlaying()){Thread.sleep(1);}
                    doneAudio.set(true);
                    audioPaused.set(true);
                    // pkt.unref();
                    continue;
                ...
                while (true) {
                    ...
                    if (ret == 0) {
                        ...
                        int maxOutSamples = (int) av_rescale_rnd(
                                swr_get_delay(swrCtx, codec.cc.sample_rate()) + inSamples,
                                outRate,
                        ...
                        if (outFmt == AV_SAMPLE_FMT_FLT) {
                            boolean queued = openAL.queuePcmF32(pcm, outRate, 2);
                            if (!queued) { openAL.pumpProcessed(); queued = openAL.queuePcmF32(pcm, outRate, 2); }
                            if (!queued) logger.warn("Failed to queue F32 audio frame");
                        } else {
                            boolean queued = openAL.queuePcm16(pcm, outRate, 2);
                            if (!queued) { openAL.pumpProcessed(); queued = openAL.queuePcm16(pcm, outRate, 2); }
                            if (!queued) logger.warn("Failed to queue S16 audio frame");
                        }
                        av_free(raw);
                        audioFrame.unref();
                        continue;
...
//    public static long nsToStreamPts(long userNs,
//                                     AVRational audioTb,
//                                     long streamStartPts) {
//        // Convert ns -> microseconds (AV_TIME_BASE units)
//        long us = userNs / 1_000L;
//
//        // Convert microseconds -> stream time_base units
//        AVRational avTimeBaseQ = new AVRational().num(1).den(AV_TIME_BASE);
//        long userPts = av_rescale_q(us, avTimeBaseQ, audioTb);
//
//        // Add stream start offset so 0 = real beginning
//        return streamStartPts + userPts;
//    }
//
//    public static long streamPtsToNs(long streamPts,
//                                     AVRational audioTb,
//                                     long streamStartPts) {
//        if (streamPts == AV_NOPTS_VALUE) return -1;
//
//        // Remove stream start offset so 0 = real beginning
//        long userPts = streamPts - streamStartPts;
//        if (userPts < 0) userPts = 0;
//
//        // Convert stream time_base units -> microseconds
//        AVRational avTimeBaseQ = new AVRational().num(1).den(AV_TIME_BASE);
//        long us = av_rescale_q(userPts, audioTb, avTimeBaseQ);
//
//        // microseconds -> ns
//        return us * 1_000L;
//    }
//
//    public static long nsToStreamTs(long ns, AVRational streamTb) {
//        // ns -> us (AV_TIME_BASE units)
//        long us = ns / 1_000L;
//
//        AVRational avTimeBaseQ = new AVRational().num(1).den(AV_TIME_BASE); // 1/1_000_000
//        return av_rescale_q(us, avTimeBaseQ, streamTb);
//    }
//
//    public static long ptsToNs(long pts, AVRational streamTb) {
//        if (pts == AV_NOPTS_VALUE) return -1;
//
//        // 1 / 1_000_000_000 (nanosecond time base)
//        AVRational nsBase = new AVRational().num(1).den(1_000_000_000);
//
//        return av_rescale_q(pts, streamTb, nsBase);
//    }
}